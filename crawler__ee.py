    ...
    for i in range(len(device_list_1)):
        ...
        for j in range(len(var_list)):
            ...
            for m in range(len(device_colour_list)):
                ...
                temp_13 = temp_12.where(temp_12['Device colour'] == dvc).dropna(axis=0, how='all')
                temp_23 = temp_22.where(temp_22['Device colour'] == dvc).dropna(axis=0, how='all')
                plan_type_list = np.unique(temp_13['Tariff Name/Plan Type']).tolist()

                for k in range(len(plan_type_list)):
                    hk = []
                    pt = plan_type_list[k]
                    idx_main = f"{device}_{var}_{dvc}_{pt}"
                    temp_14 = temp_13.where(temp_13['Tariff Name/Plan Type'] == pt).dropna(axis=0, how='all')
                    temp_24 = temp_23.where(temp_23['Tariff Name/Plan Type'] == pt).dropna(axis=0, how='all')
                    ...
                    if temp_14.shape == temp_24.shape:
                        ...
                        for f in range(len(k)):
                            hk += [
                                [col_name[k[f][1]], temp_14.iloc[k[f][0]]['row_no'], temp_24.iloc[k[f][0]]['row_no']]]
